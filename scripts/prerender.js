...
if (!fs.existsSync(distPath)) {
	console.error(
		"❌ Build directory dist/ does not exist. Please run npm run build first.",
	);
	process.exit(1);
}
...
const toolBlocks =
	registryContent.match(/\{\s*id:\s*['"][\s\S]*?component:\s*\w+\s*\}/g) || [];
const tools = toolBlocks
	.map((block) => {
		...
		const categoriesRaw =
			(block.match(/categories:\s*\[([\s\S]*?)\]/) || [])[1] || "";
		const categories = Array.from(
			categoriesRaw.matchAll(/['"]([^'"]+)['"]/g),
			(m) => m[1],
		);
		...
		return {
			...
			keywords:
				keywords ||
				`${name?.toLowerCase()}, free ${name?.toLowerCase()}, online ${name?.toLowerCase()}, ${primaryCategory} tools, domodomo`,
		};
	})
	.filter((t) => t.id);
...
tools.forEach((tool) => {
	...
	const schemaMarkup = {
		...
		operatingSystem: "All",
		browserRequirements:
			"Requires HTML5, WebAssembly, and modern browser support.",
	...
	let pageHtml = templateHtml
		...
		.replace(
			...
			`<meta property="og:url" content="${escapeHtml(toolUrl)}" />`,
		)
		// Twitter Title
		.replace(
		...
		.replace(
			...
			`<link rel="canonical" href="${escapeHtml(toolUrl)}" />`,
		);
...
const qrTypes = [
	{
		...
		desc: "Generate a WiFi QR code to share your network name and password securely.",
	},
	{
		id: "vcard-business-qr-generator",
		name: "vCard Business QR Generator",
		desc: "Create a digital business card contact QR code with your name, phone, and email.",
	},
	{
		id: "custom-logo-qr-generator",
		name: "Custom Logo QR Generator",
		desc: "Design branded QR codes online by embedding your custom logo.",
	},
	{
		id: "url-link-qr-generator",
		name: "URL Link QR Generator",
		desc: "Generate QR codes for web links, landing pages, and social media profiles.",
	},
	{
		id: "sms-text-qr-generator",
		name: "SMS Text QR Generator",
		desc: "Create QR codes that automatically open text messages with pre-filled inputs.",
	},
	{
		id: "email-qr-code-generator",
		name: "Email QR Code Generator",
		desc: "Generate QR codes that open mail apps with subject and address ready.",
	},
	// Socials
	{
		id: "whatsapp-qr-code-generator",
		name: "WhatsApp QR Code Generator",
		desc: "Generate QR codes that launch WhatsApp chats with pre-filled messages.",
	},
	{
		id: "instagram-qr-code-generator",
		name: "Instagram QR Code Generator",
		desc: "Create custom QR codes that redirect visitors directly to your Instagram profile.",
	},
	{
		id: "facebook-qr-code-generator",
		name: "Facebook Page QR Generator",
		desc: "Generate custom scannable QR codes for Facebook pages and groups.",
	},
	{
		id: "youtube-qr-code-generator",
		name: "YouTube Channel QR Generator",
		desc: "Create QR codes linking directly to your YouTube videos or channels.",
	},
	{
		id: "linkedin-qr-code-generator",
		name: "LinkedIn Profile QR Generator",
		desc: "Generate networking QR codes redirecting to your professional LinkedIn page.",
	},
	{
		id: "tiktok-qr-code-generator",
		name: "TikTok Account QR Generator",
		desc: "Create custom social sharing QR codes targeting TikTok profiles.",
	},
	{
		id: "pinterest-qr-code-generator",
		name: "Pinterest Board QR Generator",
		desc: "Generate visual QR codes linking to Pinterest boards and pins.",
	},
	// Financial
	{
		id: "paypal-qr-code-generator",
		name: "PayPal Payment QR Generator",
		desc: "Create scan-to-pay QR codes linked to your PayPal account.",
	},
	{
		id: "bitcoin-qr-code-generator",
		name: "Bitcoin Wallet QR Generator",
		desc: "Generate cryptocurrency receiving address QR codes for Bitcoin transactions.",
	},
	{
		id: "ethereum-qr-code-generator",
		name: "Ethereum Wallet QR Generator",
		desc: "Generate ERC-20 payment address QR codes for Ethereum transactions.",
	},
	{
		id: "crypto-qr-code-generator",
		name: "Crypto Address QR Generator",
		desc: "Create wallet address scanning QR cards for various blockchain networks.",
	},
];
qrTypes.forEach((t) => {
	variations.push({
...
variations.forEach((v) => {
	...
	let pageHtml = templateHtml
		...
		.replace(
			/<meta name="keywords" content=".*?" \/>/,
			`<meta name="keywords" content="${escapeHtml(v.keywords)}" />`,
		)
		.replace(
			/<meta property="og:title" content=".*?" \/>/,
			`<meta property="og:title" content="${escapeHtml(v.seoTitle)}" />`,
		)
		.replace(
			/<meta property="og:description" content=".*?" \/>/,
			`<meta property="og:description" content="${escapeHtml(seoDesc)}" />`,
		...
		.replace(
			/<meta name="twitter:title" content=".*?" \/>/,
			`<meta name="twitter:title" content="${escapeHtml(v.seoTitle)}" />`,
		)
		.replace(
			/<meta name="twitter:description" content=".*?" \/>/,
			`<meta name="twitter:description" content="${escapeHtml(seoDesc)}" />`,
...
const postBlocks =
	blogDataContent.match(
		/\{\s*slug:\s*['"][\s\S]*?content:\s*`[\s\S]*?`\s*\}/g,
	) || [];
const blogPosts = postBlocks
	.map((block) => {
...
blogPosts.forEach((post) => {
	...
	let pageHtml = templateHtml
		.replace(
			/<title>.*?<\/title>/,
			`<title>${escapeHtml(post.title)} | DomoDomo Guides</title>`,
		)
		.replace(
			/<meta name="description" content=".*?" \/>/,
    ...
    <div id="seo-fallback" style="padding: 2rem; max-width: 800px; margin: 0 auto; font-family: sans-serif; color: #ecebe9; text-align: left;">
      ...
      <div style="line-height: 1.7; color: #a3a09b;">
        ${post.content
					...
					.replace(
						/## (.*?)\n/g,
						'<h2 style="color:#ecebe9; margin-top:2rem;">$1</h2>',
					)
					.replace(
						/# (.*?)\n/g,
      ...
      <div style="text-align: center; margin-top: 3rem; border-t: 1px solid #2a2d30; pt: 2rem;">
        <a href="/" style="display: inline-block; padding: 0.75rem 1.5rem; background: #4e8e5e; color: white; text-decoration: none; border-radius: 8px; font-weight: bold;">Explore 120+ Offline Tools</a>
      </div>
    </div>
  `;
