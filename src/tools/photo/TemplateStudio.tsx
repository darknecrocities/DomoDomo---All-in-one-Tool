...
export const TemplateStudioTool = () => {
  ...
  const addShapeLayer = (shapeType: 'rect' | 'circle' | 'triangle' | 'star' | 'arrow') => {
    addLayer({
      id: `shape-${shapeType}-${Date.now()}`,
      type: 'shape',
      shapeType,
      name: `${shapeType.charAt(0).toUpperCase() + shapeType.slice(1)} ${template.layers.length + 1}`,
      ...
      innerRadius: shapeType === 'star' ? 25 : undefined,
      outerRadius: shapeType === 'star' ? 50 : undefined
    } as any);
  };
            ...
            <>
              <div className="space-y-3">
                ...
                <div className="space-y-2 border-t border-[#2A2D30] pt-3">
                  <label className="text-[10px] text-[#72706C] font-bold block">Canvas Preset Size</label>
                  <select
                    onChange={(e) => {
                      const val = e.target.value;
                      if (val === 'instagram') {
                        updateTemplate(p => ({ ...p, width: 1080, height: 1080 }));
                      } else if (val === 'presentation') {
                        updateTemplate(p => ({ ...p, width: 1920, height: 1080 }));
                      } else if (val === 'story') {
                        updateTemplate(p => ({ ...p, width: 1080, height: 1920 }));
                      } else if (val === 'a4') {
                        updateTemplate(p => ({ ...p, width: 794, height: 1123 }));
                      }
                    }}
                    className="w-full bg-[#111213] border border-[#2A2D30] rounded p-2 text-[#ECEBE9] text-xs outline-none"
                  >
                    <option value="">Custom Size / Select Preset...</option>
                    <option value="instagram">Instagram Post (1080 x 1080)</option>
                    <option value="presentation">Presentation 16:9 (1920 x 1080)</option>
                    <option value="story">Instagram Story 9:16 (1080 x 1920)</option>
                    <option value="a4">A4 Print Page (794 x 1123)</option>
                  </select>

                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <div>
                      <label className="text-[9px] text-[#72706C] font-bold block mb-0.5">WIDTH (PX)</label>
                      <input
                        type="number"
                        value={template.width}
                        onChange={e => updateTemplate(p => ({ ...p, width: Number(e.target.value) || 800 }))}
                        className="w-full bg-[#111213] border border-[#2A2D30] rounded p-1.5 text-[#ECEBE9] text-xs outline-none font-mono"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] text-[#72706C] font-bold block mb-0.5">HEIGHT (PX)</label>
                      <input
                        type="number"
                        value={template.height}
                        onChange={e => updateTemplate(p => ({ ...p, height: Number(e.target.value) || 600 }))}
                        className="w-full bg-[#111213] border border-[#2A2D30] rounded p-1.5 text-[#ECEBE9] text-xs outline-none font-mono"
                      />
                    </div>
                  </div>
                ...
                <div className="text-[10px] text-[#72706C] font-bold uppercase tracking-wider mt-4 mb-1">Add Shapes</div>
                <div className="grid grid-cols-3 gap-2">
                  <button onClick={() => addShapeLayer('rect')} className="py-1.5 bg-[#2A2D30] hover:bg-[#3A3D40] rounded-xl text-[10px] font-bold text-center">
                    Square
                  </button>
                  <button onClick={() => addShapeLayer('circle')} className="py-1.5 bg-[#2A2D30] hover:bg-[#3A3D40] rounded-xl text-[10px] font-bold text-center">
                    Circle
                  </button>
                  <button onClick={() => addShapeLayer('triangle')} className="py-1.5 bg-[#2A2D30] hover:bg-[#3A3D40] rounded-xl text-[10px] font-bold text-center">
                    Triangle
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-2 mt-2">
                  <button onClick={() => addShapeLayer('star')} className="py-1.5 bg-[#2A2D30] hover:bg-[#3A3D40] rounded-xl text-[10px] font-bold text-center">
                    Star
                  </button>
                  <button onClick={() => addShapeLayer('arrow')} className="py-1.5 bg-[#2A2D30] hover:bg-[#3A3D40] rounded-xl text-[10px] font-bold text-center">
                    Arrow
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              {template.layers.length > 0 && (
                <div className="space-y-4">
                  ...
                  {template.layers.filter(l => 'variableName' in l && (l as any).variableName).map((layer) => {
