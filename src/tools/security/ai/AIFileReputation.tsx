...
import { ShieldAlert, Cpu, Activity, CheckCircle2 } from 'lucide-react';
import { aiService } from '../../../utils/aiService';

export const AIFileReputationTool: React.FC = () => {
  ...
  const analyzeStrings = async () => {
    ...
    const systemPrompt = `You are an expert Malware Analyst.
The user will provide you with strings extracted from a Windows PE (Executable) or ELF file.
Review the strings and determine if the file is malicious.
Look for:
1. Suspicious API calls (VirtualAlloc, LoadLibrary, GetProcAddress).
2. Crypto wallet addresses or hardcoded passwords.
3. Obfuscation markers (UPX, base64 strings).
Give a final verdict (Safe, Suspicious, Malicious) and justify your reasoning.`;
    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full">
      <div className="glass-card p-6 border-[#2A2D30] bg-[#18191B] flex flex-col gap-4 text-left">
        <h3 className="font-bold text-[#ECEBE9] text-lg flex items-center gap-2">
          ...
          DomoGuard File Reputation
        </h3>
        <p className="text-[#A3A09B] text-xs leading-relaxed">
          Paste the output of `strings malware.exe` or a hex dump. Your offline AI will scan it for suspicious Win32 APIs, crypto addresses, and obfuscation.
        </p>
      </div>
      ...
      <div className="glass-card p-6 border-[#2A2D30] bg-[#111213] flex flex-col gap-4">
        ...
        >
          {isAnalyzing ? (
            ...
            <>
              <ShieldAlert size={16} />
              <span>Analyze File Reputation Locally</span>
            </>
          )}
        </button>
      </div>
      ...
      {result && (
        <div className="glass-card p-6 border-[#3C6B4D]/30 bg-[#18191B] flex flex-col gap-4 text-left">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="font-bold text-[#ECEBE9] text-sm uppercase tracking-wider">Reputation Analysis</h4>
          </div>
