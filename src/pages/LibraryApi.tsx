...
export const LibraryApi = () => {
  ...
  // Section States
  const [currentSection, setCurrentSection] = useState<'global' | 'philippine'>('global');

  const handleSectionChange = (section: 'global' | 'philippine') => {
  ...
  const activeDataset = useMemo(() => {
    return currentSection === 'global' ? PUBLIC_APIS_DATA : PH_APIS_DATA;
  }, [currentSection]);
            ...
            <div className="flex flex-col gap-1 max-h-[450px] overflow-y-auto pr-1">
              {categories.map((cat) => {
                const count = cat === 'All' 
                  ...
                  : activeDataset.filter(a => a.category === cat).length;
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="flex items-center justify-between">
            <p className="text-xs font-mono text-[#72706C]">
              Showing {filteredApis.length} of {activeDataset.length} results
            </p>
          </div>
