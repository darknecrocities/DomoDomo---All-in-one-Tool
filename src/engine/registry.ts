...
export const TOOLS: Tool[] = [
  ...
  { id: 'ai-enhancer', name: 'Image Detail Enhancer', categories: ['photo'], description: 'Enhance photo details using local contrast and color filters.', icon: 'Image', run: async (i) => i, component: AIImageEnhancerTool },
  ...
  // Smart Assistants (20)
  { id: 'ai-chat', name: 'Chat & Writing Assistant', categories: ['ai'], description: 'Chat offline with your local smart assistant.', icon: 'Hammer', run: async (i) => i, component: LocalAIChatTool },
  { id: 'ai-summarizer', name: 'Document Summarizer', categories: ['ai'], description: 'Summarize text documents using local document parsing.', icon: 'Hammer', run: async (i) => i, component: AISummarizerTool },
  { id: 'caption-gen', name: 'Caption Generator', categories: ['ai'], description: 'Generate clean descriptive image captions.', icon: 'Hammer', run: async (i) => i, component: AICaptionTool },
  { id: 'ocr-assistant', name: 'OCR Assistant', categories: ['ai'], description: 'Format OCR results into clean structured layouts.', icon: 'Hammer', run: async (i) => i, component: AIOCRAssistantTool },
  { id: 'prompt-enhancer', name: 'Prompt & Request Enhancer', categories: ['ai'], description: 'Enhance simple requests into clear, detailed instructions.', icon: 'Hammer', run: async (i) => i, component: AIPromptEnhancerTool },
  { id: 'image-classifier', name: 'Image Classifier', categories: ['ai'], description: 'Classify visual items of uploaded images locally.', icon: 'Hammer', run: async (i) => i, component: AIClassifierTool },
  { id: 'text-rewriter', name: 'Text Rewriter', categories: ['ai'], description: 'Rewrite text into corporate, casual, or concise tones.', icon: 'Hammer', run: async (i) => i, component: AITextRewriterTool },
  { id: 'ai-translator', name: 'Multilingual Translator', categories: ['ai'], description: 'Translate text offline with zero network latency.', icon: 'Hammer', run: async (i) => i, component: AITranslatorTool },
  { id: 'ai-stt', name: 'Speech-to-Text Dictation', categories: ['ai'], description: 'Transcribe spoken audio inputs locally on your machine.', icon: 'Hammer', run: async (i) => i, component: AISpeechToTextTool },
  ...
  { id: 'ai-code-explainer', name: 'Code Explainer & Inspector', categories: ['ai'], description: 'Paste code to get plain-English explanations, complexity scoring, and language translations.', icon: 'Hammer', run: async (i) => i, component: AICodeExplainerTool },
  { id: 'ai-flashcard-maker', name: 'Study Flashcard Generator', categories: ['ai'], description: 'Turn study materials and notes into organized Q&A flashcards.', icon: 'Hammer', run: async (i) => i, component: AIFlashcardMakerTool },
  { id: 'ai-sentiment-journal', name: 'Reflective Journal & Sentiment Tracker', categories: ['ai'], description: 'Write journal entries and track mood and sentiment trends privately.', icon: 'Hammer', run: async (i) => i, component: AISentimentJournalTool },
  { id: 'ai-email-composer', name: 'Professional Email Drafter', categories: ['ai'], description: 'Draft polished, professional emails from bullet points or intents.', icon: 'Hammer', run: async (i) => i, component: AIEmailComposerTool },
  { id: 'ai-story-generator', name: 'Creative Story Drafter', categories: ['ai'], description: 'Generate short stories from genre, character, and setting prompts.', icon: 'Hammer', run: async (i) => i, component: AIStoryGeneratorTool },
  { id: 'ai-debate-assistant', name: 'Debate & Perspective Assistant', categories: ['ai'], description: 'Given a topic, generate balanced pro/con arguments, counterpoints, and rebuttals.', icon: 'Hammer', run: async (i) => i, component: AIDebateAssistantTool },
  { id: 'ai-math-solver', name: 'Math Problem Solver', categories: ['ai'], description: 'Paste math problems and get verified, step-by-step solutions.', icon: 'Hammer', run: async (i) => i, component: AIMathSolverTool },
  { id: 'ai-recipe-generator', name: 'Kitchen Recipe Assistant', categories: ['ai'], description: 'Ingredients in → full recipe with cooking steps, nutrition, and variants.', icon: 'Hammer', run: async (i) => i, component: AIRecipeGeneratorTool },
  { id: 'ai-code-reviewer', name: 'Code Quality & Security Reviewer', categories: ['ai'], description: 'Review code for bugs, security vulnerabilities, and performance improvements.', icon: 'Hammer', run: async (i) => i, component: AICodeReviewerTool },
  { id: 'ai-mind-mapper', name: 'Topic & Mind Mapper', categories: ['ai'], description: 'Turn any concept or topic into an organized outline and visual mind map.', icon: 'Hammer', run: async (i) => i, component: AIMindMapperTool },
  { id: 'domo-agent-hub', name: 'Domo Coding Workspace', categories: ['ai'], description: 'Interactive offline coding environment and workspace assistant.', icon: 'Hammer', run: async (i) => i, component: AIDomoAgentHub },
  { id: 'domo-selection', name: 'Selection Explainer', categories: ['ai'], description: 'Highlight text or code to query the offline assistant instantly.', icon: 'Hammer', run: async (i) => i, component: AIDomoSelection },
  { id: 'domo-local-brain', name: 'Local Knowledge Base & Memory', categories: ['ai'], description: 'Manage your unified local database and configure private learning settings.', icon: 'Brain', run: async (i) => i, component: DomoBrainControlCenter },
  { id: 'domo-cognitive-mapper', name: 'Knowledge Mind Mapper', categories: ['ai'], description: 'Explore connected knowledge bases, recent activities, and system tools in 3D.', icon: 'Brain', run: async (i) => i, component: DomoMindMapperTool },
  { id: 'ollama-library', name: 'Local Model Library', categories: ['ai'], description: 'Browse, compare, and install local offline models with live indicators.', icon: 'Hammer', run: async (i) => i, component: AIDomoModelLibrary },
  { id: 'domo-skill-creator', name: 'Skill & Capability Creator', categories: ['ai'], description: 'Design structured capabilities, instructions, and behaviors visually.', icon: 'Hammer', run: async (i) => i, component: DomoSkillCreatorTool },
  { id: 'domoskills', name: 'DomoSkills Marketplace', categories: ['ai', 'dev'], description: 'Discover, inspect, and install 200+ verified modular capabilities for coding assistants.', icon: 'Sparkles', run: async (i) => i, component: DomoSkillsMarketplaceTool },
  { id: 'auto-pilot', name: 'Auto-Pilot Task Workspace', categories: ['ai'], description: 'Autonomous workflow assistant that executes voice and text requests.', icon: 'Cpu', run: async (i) => i, component: AutoPilotWorkspace },
  { id: 'model-migrator', name: 'Model Backup & Migrator', categories: ['ai'], description: 'Back up local models to external storage and restore them offline.', icon: 'HardDrive', run: async (i) => i, component: ModelMigrator },
  { id: 'domo-neural-atlas', name: 'Context & Memory Atlas', categories: ['ai'], description: 'Interactive 2D visual layout map of local context chunks and memory nodes.', icon: 'Brain', run: async (i) => i, component: DomoNeuralAtlas },
  { id: 'domo-flow-editor', name: 'Workflow Pipeline Editor', categories: ['ai'], description: 'Construct and wire multi-stage automated processing pipelines visually.', icon: 'Cpu', run: async (i) => i, component: DomoFlowEditor },
  { id: 'ai-sandbox-arena', name: 'Model Benchmark Arena', categories: ['ai'], description: 'Compare Time-To-First-Token, generation speed, and responses side-by-side.', icon: 'Layers', run: async (i) => i, component: AISandboxArena },
  { id: 'ai-voice-companion', name: 'Voice Companion Simulator', categories: ['ai'], description: 'Offline vocal conversation simulator with speech recognition and audio synthesis.', icon: 'Volume2', run: async (i) => i, component: AIVoiceCompanion },
  { id: 'ai-pdf-investigator', name: 'PDF Document Investigator', categories: ['ai'], description: 'Multi-document question answering tool with page-level context citations.', icon: 'BookOpen', run: async (i) => i, component: AIPDFInvestigator },
  { id: 'ai-research-orchestration-hub', name: 'Research Campaign Orchestrator', categories: ['ai'], description: 'Orchestrate structured research campaigns with specialized local assistants and conflict checks.', icon: 'Brain', run: async (i) => i, component: AIResearchOrchestrationHub },
  { id: 'ai-pii-redactor', name: 'PII Data Redactor', categories: ['ai'], description: 'Locally redact sensitive info (names, IPs, emails) from logs and documents.', icon: 'Shield', run: async (i) => i, component: AIPiiRedactorTool },
  { id: 'ai-regex-architect', name: 'Regex Explainer & Builder', categories: ['ai'], description: 'Explain complex regex patterns or generate them from plain English specifications.', icon: 'Code', run: async (i) => i, component: AIRegexArchitectTool },
  { id: 'ai-devops-architect', name: 'DevOps Command Architect', categories: ['ai'], description: 'Generate bash commands, Dockerfiles, and cron syntax from plain descriptions.', icon: 'Terminal', run: async (i) => i, component: AIDevOpsArchitectTool },
  { id: 'ai-hub-studio', name: 'Studio Lab (Fine-Tune & Flow Automation)', categories: ['ai'], description: 'Local workspace featuring interactive chat, model hub, multi-method fine-tuning, and flow automations.', icon: 'Bot', run: async (i) => i, component: AIHubStudio },
  { id: 'ai-model-manager', name: 'Model Settings & Storage Manager', categories: ['ai'], description: 'Manage local model weights, delete unused files, inspect configurations, and control VRAM allocation.', icon: 'Settings', run: async (i) => i, component: AIHubStudio },
  ...
  { id: 'wcag-contrast-checker', name: 'WCAG Contrast & Accessibility Auditor', categories: ['design', 'ai'], description: 'Evaluate text-to-background contrast ratios against WCAG 2.1 AA/AAA standards and inspect accessibility improvements locally.', icon: 'Palette', run: async (i) => i, component: WCAGContrastCheckerTool },
  { id: 'mesh-gradient-generator', name: 'CSS Glass & Mesh Gradient Studio', categories: ['design', 'ai'], description: 'Synthesize fluid mesh gradients, glassmorphism cards, and CSS backdrop filters visually with real-time CSS export.', icon: 'Sparkles', run: async (i) => i, component: MeshGradientGeneratorTool },
  { id: 'font-glyph-inspector', name: 'Font Subsetter & Typography Advisor', categories: ['design', 'ai'], description: 'Inspect Unicode character glyph maps, define lightweight font subset ranges, and preview matching typography pairings.', icon: 'Type', run: async (i) => i, component: FontGlyphInspectorTool },
  { id: 'svg-path-studio', name: 'SVG Component & Vector Code Studio', categories: ['design', 'ai'], description: 'Edit Bezier path coordinates interactively and generate clean, animated React SVG components.', icon: 'PenTool', run: async (i) => i, component: SVGPathStudioTool },
  { id: 'neumorphism-glass-builder', name: 'Neumorphism & Soft UI Generator', categories: ['design', 'ai'], description: 'Design soft extruded and inset neumorphic UI components, generate dual light/dark shadows, and synthesize soft UI systems.', icon: 'Layers', run: async (i) => i, component: NeumorphismGlassBuilderTool },
  { id: 'box-shadow-studio', name: 'Multi-Layer CSS Shadow & Elevation Studio', categories: ['design', 'ai'], description: 'Compose multi-layer ambient and key shadows, build realistic natural elevation steps, and configure lighting parameters.', icon: 'Layers', run: async (i) => i, component: BoxShadowStudioTool },
  { id: 'color-palette-harmonies', name: 'Color Palette & Harmony Studio', categories: ['design', 'ai'], description: 'Calculate color theory harmonies (Complementary, Triadic, Analogous), generate Tailwind theme configs, and check contrast swatches.', icon: 'Palette', run: async (i) => i, component: ColorPaletteHarmoniesTool },
  { id: 'border-radius-blob-maker', name: 'Organic CSS Blob & Border Radius Generator', categories: ['design', 'ai'], description: 'Manipulate 8-point organic CSS border-radius coordinates, synthesize fluid morphing keyframes, and copy production-ready CSS.', icon: 'Wand2', run: async (i) => i, component: BorderRadiusBlobMakerTool },
  { id: 'css-grid-flex-studio', name: 'CSS Grid & Flexbox Layout Architect', categories: ['design', 'ai'], description: 'Visual CSS Grid and Flexbox container layout builder, customize gap, alignment, and template columns with live preview.', icon: 'LayoutGrid', run: async (i) => i, component: CSSGridFlexStudioTool },
  { id: 'design-token-generator', name: 'Design Token & CSS Variables Studio', categories: ['design', 'ai'], description: 'Construct cross-platform design token suites (Colors, Typography scale, Spacing, Elevation) and export W3C JSON / CSS custom variables.', icon: 'Sliders', run: async (i) => i, component: DesignTokenGeneratorTool },

  // Network Suite Tools (10)
  { id: 'har-waterfall-profiler', name: 'HAR Performance Diagnostic Profiler', categories: ['network', 'ai'], description: 'Upload .har browser network traces offline to inspect request latency waterfalls and identify performance bottlenecks.', icon: 'BarChart3', run: async (i) => i, component: HARWaterfallProfilerTool },
  { id: 'curl-code-studio', name: 'cURL to Multi-Language SDK Generator', categories: ['network', 'ai'], description: 'Translate raw cURL requests into production-ready JavaScript, Python, Go, Rust, and PHP code snippets or client wrappers.', icon: 'Terminal', run: async (i) => i, component: CurlCodeStudioTool },
  { id: 'csp-header-builder', name: 'Security Header & CSP Policy Architect', categories: ['network', 'security', 'ai'], description: 'Configure Content Security Policy (CSP) directives visually, audit XSS risks, and generate robust server headers.', icon: 'ShieldCheck', run: async (i) => i, component: CSPHeaderBuilderTool },
  { id: 'jwt-debugger-studio', name: 'JWT Token Inspector & Claims Auditor', categories: ['network', 'security', 'ai'], description: 'Decode JSON Web Tokens (Header, Payload, Signature) offline, inspect expiration claims, and verify signing algorithms.', icon: 'KeyRound', run: async (i) => i, component: JWTDebuggerStudioTool },
  { id: 'websocket-tester-studio', name: 'WebSockets & SSE Packet Diagnostic Studio', categories: ['network', 'ai'], description: 'Inspect real-time WebSocket frames, stream Server-Sent Events (SSE), and test reconnect backoff latency with payload inspection.', icon: 'Terminal', run: async (i) => i, component: WebSocketTesterStudioTool },
  { id: 'dns-lookup-inspector', name: 'DNS & DoH Security Diagnostic Inspector', categories: ['network', 'security', 'ai'], description: 'Perform encrypted DNS-over-HTTPS (DoH) record queries (A, AAAA, MX, TXT, CAA, NS) and audit SPF/DMARC spoofing risks.', icon: 'ShieldCheck', run: async (i) => i, component: DNSLookupInspectorTool },
  { id: 'http-status-debugger', name: 'HTTP Status & CORS Diagnostic Studio', categories: ['network', 'ai'], description: 'Lookup HTTP 1xx-5xx status codes, debug CORS headers, and generate Express/Nginx CORS middleware configs.', icon: 'ShieldCheck', run: async (i) => i, component: HTTPStatusDebuggerTool },
  { id: 'graphql-query-studio', name: 'GraphQL Schema & Query Optimizer', categories: ['network', 'ai'], description: 'Format GraphQL queries, evaluate nested field depth complexity, and generate TypeScript response types.', icon: 'Terminal', run: async (i) => i, component: GraphQLQueryStudioTool },
  { id: 'ip-subnet-calculator', name: 'IP CIDR & Subnet Calculator', categories: ['network', 'ai'], description: 'Calculate IPv4/IPv6 CIDR network address boundaries, wildcard netmasks, usable host counts, and network subnet plans.', icon: 'Terminal', run: async (i) => i, component: IPSubnetCalculatorTool },
  { id: 'open-api-schema-builder', name: 'OpenAPI 3.0 & Swagger Schema Studio', categories: ['network', 'ai'], description: 'Construct OpenAPI 3.0 JSON/YAML specifications, generate TypeScript API response contracts, and export Swagger schemas.', icon: 'Terminal', run: async (i) => i, component: OpenAPISchemaBuilderTool },

  // Additional Smart & Data Tools (3)
  { id: 'bip39-wallet-inspector', name: 'Cryptographic BIP-39 & Security Auditor', categories: ['security', 'ai'], description: 'Inspect BIP-39 seed mnemonics, verify HD wallet derivation paths, and audit cryptographic entropy offline.', icon: 'KeyRound', run: async (i) => i, component: BIP39WalletInspectorTool },
  { id: 'srt-subtitle-tool', name: 'Subtitle Translator & Smart Re-timer', categories: ['converter', 'video', 'ai'], description: 'Edit SRT/VTT subtitle timing offsets (+/- ms), re-sync subtitle drift, and translate subtitles line-by-line preserving timestamp markup locally.', icon: 'Subtitles', run: async (i) => i, component: SRTSubtitleTool },
  { id: 'csv-schema-diff', name: 'CSV Schema & Data Drift Diagnostic Tool', categories: ['data', 'ai'], description: 'Compare two CSV/TSV spreadsheets column-by-column, detect structural schema shifts, and generate database migration SQL scripts.', icon: 'FileSpreadsheet', run: async (i) => i, component: CSVSchemaDiffTool },
